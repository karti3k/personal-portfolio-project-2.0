...
const About: React.FC<AboutProps> = ({ isDarkMode }) => {
    ...
    return (
        <main id='about-section' className={`w-full lg:h-screen md:h-auto h-max flex md:gap-0 flex-col md:pt-32  md:pb-44 lg:pt-0 lg:pb-0 ${isDarkMode ? 'bg-custom-light-black' : 'bg-custom-blue'}`}>
            <span className={`px-8 md:px-12 mt-14 md:mt-0 lg:mt-24 lg:px-32 text-center anekDevanagari-font text-3xl md:text-4xl font-semibold opacity-80 h-16 drop-shadow-lg ${isDarkMode ? 'text-custom-blue' : 'text-white'}`}><u>About</u>:</span>
